...
let rec sin x =
  if fless x 0.0 then fneg (sin (fneg x)) else
  if fless (pi /. 2.0) x then sin (pi -. x) else
  let rec g n =
    let m = 2 * n + 1 in
...
let rec cos x =
  if fless x 0.0 then cos (fneg x) else
  if fless (pi /. 2.0) x then fneg (cos (pi -. x)) else
  let rec g n =
    let m = 2 * n in
    let f = pow x m /. float_of_int (fact m) in
    if even n then f else fneg f
