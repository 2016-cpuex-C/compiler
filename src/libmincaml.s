...
min_caml_array_init:	# write $a0 in ($v0)~($v0+$v1-1), return $v0 itself
	...
	li	$a1, 0
min_caml_create_array_loop_:
	bne	$v1, $a1, min_caml_create_array_cont_
min_caml_create_array_exit_:
	jr	$ra
min_caml_create_array_cont_:
	sw	$a0, 0($a2)
	addi	$v1, $v1, -1
	addi	$a2, $a2, 1
	j	min_caml_create_array_loop_
...
min_caml_float_array_init:	# write $f0 in ($v0)~($v0+$v1-1), return $v0 itself
	...
	move	$a2, $v0 # memory counter
	li	$a1, 0
min_caml_create_float_array_loop_:
	bne	$v1, $a1, min_caml_create_float_array_cont_
min_caml_create_float_array_exit_:
	jr	$ra
min_caml_create_float_array_cont_:
	s.s	$f0, 0($a2)
	addi	$v1, $v1, -1
	addi	$a2, $a2, 1
	j	min_caml_create_float_array_loop_
